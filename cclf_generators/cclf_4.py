...
for month in range(number_of_file_months):
    ...
    for _ in range(number_of_lines_per_file):
        ...
        contents += random_choice_from_array(["0", "9", "U"])   # CLM_PROD_TYPE_CD
        contents += random_int_by_len(2)      # CLM_VAL_SQNC_NUM
        contents += random_int_by_len(7)      # CLM_DGNS_CD
        contents += random_int_by_len(11)     # BENE_EQTBL_BIC_HICN_NUM
        contents += random_alpha_string(6)    # PRVDR_OSCAR_NUM
        ...
        contents += random_int_by_len(7)      # CLM_POA_IND
